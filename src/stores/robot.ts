...
export interface SSHOutputEntry { type: 'cmd' | 'out' | 'err' | 'hint'; text: string }
...
export interface CameraInfo { id: number; name: string; status: string; resolution: string; stream_url?: string }
export interface GimbalState { pan: number; tilt: number }
export interface WifiNetwork { ssid: string; signal: number; security: string; connected: boolean }
export interface WifiScanResult { currentSsid: string; currentDevice: string; networks: WifiNetwork[] }
