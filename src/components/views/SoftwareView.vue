...
function searchAvailable() {
  ...
  if (!q) {
    availableResults.value = robotStore.softwareAvailable
  } else {
    availableResults.value = robotStore.softwareAvailable.filter(
      ...
      <div class="software-content">
        ...
        <div class="tab-panel" :class="{ active: activeTab === 'installed' }">
          <div class="software-toolbar">
            <input type="text" v-model="installedSearch" placeholder="搜索已安装软件..." />
            <button @click="installedSearch = ''">刷新</button>
          </div>
          <div class="software-list">
            <div v-if="filteredInstalled.length === 0" class="empty-state">无匹配软件</div>
            <div
              v-for="sw in filteredInstalled"
              :key="sw.name"
              class="software-card"
            >
              ...
              <div class="software-card-info">
                ...
                <div class="software-card-meta">{{ sw.version }} &middot; {{ sw.size }} &middot; {{ sw.description }}</div>
              </div>
        ...
        <div class="tab-panel" :class="{ active: activeTab === 'available' }">
          <div class="software-toolbar">
            <input type="text" v-model="availableSearch" placeholder="搜索互联网软件包..." @keydown.enter="searchAvailable" />
...
.software-layout { display: flex; flex-direction: column; height: 100%; }
.software-tabs { display: flex; gap: 8px; margin-bottom: 16px; }
.tab-btn {
...
.software-content { flex: 1; overflow: hidden; }
.software-toolbar { display: flex; gap: 8px; margin-bottom: 12px; }
...
.software-toolbar button:hover { background: var(--bg-hover); }
.software-list { display: flex; flex-direction: column; gap: 8px; overflow-y: auto; flex: 1; }
...
.software-card-icon { font-size: 28px; }
.software-card-info { flex: 1; }
.software-card-name { font-weight: 600; font-size: 14px; }
.software-card-meta { font-size: 11px; color: var(--text-muted); margin-top: 2px; }
...
.software-card-actions button:hover { border-color: var(--accent); color: var(--accent); }
.software-card-actions button.install { background: var(--accent); border-color: var(--accent); color: var(--bg-primary); }
.software-card-actions button.uninstall { color: var(--danger); border-color: var(--danger); }
.software-card-actions button.upgrade { color: var(--warning); border-color: var(--warning); }
