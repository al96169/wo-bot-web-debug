    ...
    <div class="status-detailed">
      ...
      <div class="status-section">
        ...
        <div class="status-run-cards">
          ...
          <div class="run-card wifi-clickable" @click="showWifiManager = true" title="点击管理 WiFi">
            <div class="run-card-icon">📶</div>
      ...
      <div class="status-section">
        ...
        <div class="detail-list">
          <div
            v-for="detail in robotStore.deviceDetails"
            :key="detail.label"
            class="detail-item"
          >
            ...
            <span class="detail-item-value">
              ...
              >{{ copiedLabel === detail.label ? '✓' : '📋' }}</button>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="status-section">
        ...
        <div class="subsystem-list">
          <div
            v-for="sub in robotStore.subsystemStatus"
            :key="sub.name"
...
.status-detailed { display: flex; flex-direction: column; gap: 24px; }
.status-section h3 { margin-bottom: 12px; font-size: 16px; }
.status-run-cards { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 12px; }
.run-card {
  padding: 16px; background: var(--bg-card); border: 1px solid var(--border);
  border-radius: var(--radius-md); text-align: center;
}
.run-card-icon { font-size: 24px; margin-bottom: 8px; }
.run-card-value { font-size: 20px; font-weight: 600; margin-bottom: 4px; }
.run-card-label { font-size: 11px; color: var(--text-muted); }
...
.detail-item-label { color: var(--text-secondary); display: flex; align-items: center; gap: 8px; }
.detail-item-value { font-family: monospace; color: var(--text-primary); display: flex; align-items: center; gap: 8px; }
.copy-btn {
...
.copy-btn.copied { background: var(--success); border-color: var(--success); color: var(--bg-primary); }
...
.env-label { display: block; font-size: 12px; color: var(--text-muted); margin-bottom: 8px; }
.env-value { font-size: 24px; font-weight: 600; }
...
.subsystem-name { display: flex; align-items: center; gap: 8px; }
.subsystem-status { font-size: 12px; padding: 2px 8px; border-radius: var(--radius-sm); }
.subsystem-status.online { background: rgba(0, 255, 136, 0.2); color: var(--success); }
.subsystem-status.offline { background: rgba(255, 71, 87, 0.2); color: var(--danger); }
.wifi-clickable { cursor: pointer; transition: border-color 0.15s, box-shadow 0.15s; }
.wifi-clickable:hover { border-color: var(--accent); box-shadow: 0 0 0 1px var(--accent); }
</style>
