...
function addMockCmdLogs() {
  if (robotStore.cmdLogs.length === 0) {
    ...
    robotStore.addCmdLog({
      ...
      direction: "recv",
      type: "ACK",
      data: '{"ok":true,"data":{"battery":78}}',
    });
    robotStore.addCmdLog({
      time: "10:30:16",
      direction: "send",
      type: "CMD",
      data: '{"cmd":"move","dir":"forward","speed":50}',
    });
  ...
  <div class="bottom-panel" :class="{ expanded: appStore.bottomPanelExpanded }">
    <div class="bottom-tab-bar">
      <div class="bottom-tab-buttons">
        ...
        <button class="bottom-tab" :class="{ active: appStore.bottomPanelTab === 'ssh' }" @click="switchTab('ssh')">
          💻 SSH 终端 <span class="ssh-dot" :class="{ connected: appStore.sshConnected }"></span>
        </button>
      </div>
      <button class="bottom-panel-toggle" title="折叠面板" @click="togglePanel">━</button>
    </div>
    <div class="bottom-panel-content">
      ...
      <div class="bottom-panel-pane" :class="{ active: appStore.bottomPanelTab === 'cmdLog' }">
        ...
        <div ref="cmdLogContainerRef" class="cmd-log-container">
          <div v-if="robotStore.cmdLogs.length === 0" class="empty-state" style="padding: 24px">暂无指令记录</div>
          <div v-for="(entry, idx) in robotStore.cmdLogs" :key="idx" class="cmd-log-entry">
          ...
          <span class="ssh-status" style="display: flex; align-items: center; gap: 6px">
            <span
              :style="{
                ...
                height: '8px',
                borderRadius: '50%',
                background: sshReady ? 'var(--success)' : 'var(--danger)',
                display: 'inline-block',
              }"
            ></span>
            <span>{{
              sshReady
                ? `Terminal · ${robotStore.shellCwd}`
                : appStore.connection === "connected"
                  ? "就绪"
                  : "未连接设备"
            }}</span>
          </span>
          <button style="font-size: 11px" @click="robotStore.clearSSHOutput()">清屏</button>
        </div>
        <div ref="sshTerminalRef" class="ssh-terminal-output">
...
.ssh-input-bar input::placeholder {
  color: var(--text-muted);
}
...
.ssh-input-bar button:hover {
  background: #00b8e0;
}
.ssh-input-bar button:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
.ssh-cmd {
