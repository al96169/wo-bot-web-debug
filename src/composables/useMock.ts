...
export function useMock() {
  ...
  function generateMockModules(): void {
    const statuses: Array<Module['status']> = ['online', 'online', 'online', 'online', 'online', 'online', 'offline', 'disabled']
  ...
  function generateMockSoftware(): void {
    const swList: Software[] = [
      { name: 'wo-nav-core', version: '2.1.0', size: '45 MB', installDate: '2025-03-15', icon: 'nav', source: 'official' },
      { name: 'wo-vision-sdk', version: '1.8.3', size: '128 MB', installDate: '2025-04-02', icon: 'vision', source: 'official' },
      { name: 'wo-slam-engine', version: '3.0.1', size: '256 MB', installDate: '2025-05-10', icon: 'slam', source: 'official' },
      { name: 'community-utils', version: '0.9.5', size: '8 MB', installDate: '2025-05-22', icon: 'utils', source: 'community' },
      { name: 'debug-tools', version: '1.2.0', size: '12 MB', installDate: '2025-05-28', icon: 'debug', source: 'community' },
    ]
  ...
  function startStatusTimer(): void {
    statusTimer = setInterval(() => {
      ...
      robotStore.setSystemStatus({
        battery: { level: Math.round(mockBattery.value), status: Math.random() > 0.3 ? 'discharging' : 'charging', state: mockBattery.value > 50 ? '正常' : '低电量', temp: 25 + Math.round(Math.random() * 15) },
        ...
        environment: { temperature: `${22 + Math.round(Math.random() * 8)}°C`, humidity: `${45 + Math.round(Math.random() * 20)}%`, gas: '正常', light: `${300 + Math.round(Math.random() * 500)} lux` },
